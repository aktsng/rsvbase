<template>
  <AdminLayout>
    <Head :title="`${form.name} の編集`" />
    <template #header>施設編集: {{ form.name }}</template>

    <div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <form @submit.prevent="submit" class="space-y-8 bg-white p-8 shadow-sm ring-1 ring-slate-200 rounded-2xl">
        <!-- 基本情報 -->
        <section>
          <h3 class="text-lg font-bold text-slate-900 mb-4 border-l-4 border-blue-600 pl-3">基本情報</h3>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
              ...
              <textarea v-model="form.description" rows="4" class="block w-full px-4 py-2 border border-slate-200 rounded-xl leading-5 bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition"></textarea>
        ...
        <!-- 連絡先・住所 -->
        <section>
          <h3 class="text-lg font-bold text-slate-900 mb-4 border-l-4 border-blue-600 pl-3">連絡先・住所</h3>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-1">郵便番号</label>
              <input type="text" v-model="form.postal_code" class="block w-full px-4 py-2 border border-slate-200 rounded-xl leading-5 bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition">
            ...
            <div>
              ...
              <input type="text" v-model="form.phone" class="block w-full px-4 py-2 border border-slate-200 rounded-xl leading-5 bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition">
            ...
            <div class="md:col-span-2">
              ...
              <input type="text" v-model="form.address" class="block w-full px-4 py-2 border border-slate-200 rounded-xl leading-5 bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition">
        ...
        <!-- 運用設定 -->
        <section>
          <h3 class="text-lg font-bold text-slate-900 mb-4 border-l-4 border-blue-600 pl-3">運用設定</h3>
        ...
        <div class="flex items-center justify-end gap-4 border-t border-slate-200 pt-6">
          <Link :href="route('admin.facilities.show', facility.uuid)" class="text-sm font-semibold text-slate-600 hover:text-slate-900 transition">キャンセル</Link>
...
const form = useForm({
    ...
    same_day_booking_cutoff_time: props.facility.same_day_booking_cutoff_time.substring(0, 5),
    check_in_start_time: props.facility.check_in_start_time.substring(0, 5),
    check_in_end_time: props.facility.check_in_end_time.substring(0, 5),
    platform_fee_rate: props.facility.platform_fee_rate,
    is_published: props.facility.is_published,
});
...
const submit = () => {
    form.put(route('admin.facilities.update', props.facility.uuid));
};
</script>
