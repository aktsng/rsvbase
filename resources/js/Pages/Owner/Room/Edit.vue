    ...
    <div class="max-w-4xl mx-auto py-6">
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
        <form @submit.prevent="submit" class="space-y-6">
              ...
              <template v-if="showChildSettings">
                <div class="md:col-span-2 mt-2 p-4 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input v-model="form.child_a_label" type="text" class="block w-full px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm" placeholder="例：小学生">
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-slate-600 mb-1">ポリシー（表示テキスト）</label>
                      <input v-model="form.child_a_policy" type="text" class="block w-full px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm" placeholder="例：12歳まで・定員に含む">
                    </div>
                    <div>
                      ...
                      <div class="relative">
                        ...
                        <input v-model="form.add_child_a_fee" type="number" min="0" class="block w-full pl-8 pr-3 py-2 border border-slate-200 rounded-lg bg-white text-sm" placeholder="3000">
                      </div>
                    </div>
                    <div class="flex items-center">
                      <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" v-model="form.child_a_is_counted" class="w-4 h-4 text-primary-600 border-slate-300 rounded focus:ring-primary-500">
                ...
                <div class="md:col-span-2 p-4 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label class="block text-sm font-medium text-slate-600 mb-1">ラベル名</label>
                      <input v-model="form.child_b_label" type="text" class="block w-full px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm" placeholder="例：乳幼児">
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-slate-600 mb-1">ポリシー（表示テキスト）</label>
                      <input v-model="form.child_b_policy" type="text" class="block w-full px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm" placeholder="例：3歳まで・添い寝">
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-slate-600 mb-1">追加料金 (1名)</label>
                      <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span class="text-slate-500 text-sm">¥</span></div>
                        <input v-model="form.add_child_b_fee" type="number" min="0" class="block w-full pl-8 pr-3 py-2 border border-slate-200 rounded-lg bg-white text-sm" placeholder="0">
                      </div>
                      <p v-if="Number(form.add_child_b_fee) === 0" class="text-xs text-green-600 mt-1">※ 無料</p>
                    </div>
                    <div class="flex items-center">
                      <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" v-model="form.child_b_is_counted" class="w-4 h-4 text-primary-600 border-slate-300 rounded focus:ring-primary-500">
                        <span class="text-sm text-slate-700">定員にカウントする</span>
                      </label>
                    </div>
                  </div>
          ...
          <div class="flex flex-col-reverse md:flex-row justify-between pt-6 border-t border-slate-100 gap-4 md:gap-0 mt-8">
            <button type="button" @click="confirmDelete"
                    class="px-6 py-3 text-red-600 font-medium hover:bg-red-50 rounded-xl transition text-center md:text-left">
              この部屋を削除
            </button>
            <div class="flex flex-col md:flex-row gap-3">
              <Link :href="route('owner.rooms.index')" class="px-6 py-3 text-slate-600 font-medium hover:bg-slate-50 rounded-xl transition text-center">
                キャンセル
              </Link>
              <button type="submit" :disabled="form.processing"
                      class="px-8 py-3 bg-primary-600 text-white font-semibold rounded-xl hover:bg-primary-700 transition disabled:opacity-50 text-center">
...
const showChildSettings = ref(!!(props.room.child_a_label || props.room.child_b_label));

const form = useForm({
    _method: 'PUT',
    name: props.room.name || '',
    capacity: props.room.capacity || 2,
    min_guests: props.room.min_guests || 1,
    base_price_per_night: props.room.base_price_per_night || '',
    weekend_price_per_night: props.room.weekend_price_per_night || '',
    cleaning_fee: props.room.cleaning_fee || 0,
    description: props.room.description || '',
    is_active: props.room.is_active == 1,
    pricing_type: props.room.pricing_type || 'room',
    child_a_label: props.room.child_a_label || '小学生',
    child_a_policy: props.room.child_a_policy || '12歳まで・定員に含む',
    child_a_is_counted: props.room.child_a_is_counted ?? true,
    add_child_a_fee: props.room.add_child_a_fee || 0,
    child_b_label: props.room.child_b_label || '乳幼児',
    child_b_policy: props.room.child_b_policy || '3歳まで・添い寝',
    child_b_is_counted: props.room.child_b_is_counted ?? false,
    add_child_b_fee: props.room.add_child_b_fee || 0,
    image1: null,
    image2: null,
    image3: null,
});
...
onMounted(() => {
    if (props.room.image_urls) {
        props.room.image_urls.forEach((url, index) => {
            if (index < 3) previews.value[index] = url;
        });
    }
});
...
const submit = () => {
    form.post(route('owner.rooms.update', props.room.uuid), {
        forceFormData: true,
    });
};

const confirmDelete = () => {
    if (confirm('この部屋を削除してもよろしいですか？\n※予約が既に入っている部屋は削除できません。')) {
        router.delete(route('owner.rooms.destroy', props.room.uuid));
    }
};
</script>
